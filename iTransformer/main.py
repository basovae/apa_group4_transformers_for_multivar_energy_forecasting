...
def long_term_forecast(args, train_loader, test_loader):
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            exp.train(train_loader, setting)

            print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
            exp.test(test_loader, setting)
        ...
        print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
        exp.test(test_loader, setting, test=1)
        torch.cuda.empty_cache()
