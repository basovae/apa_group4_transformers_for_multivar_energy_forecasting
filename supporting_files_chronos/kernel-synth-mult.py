...
from sklearn.gaussian_process.kernels import (
    RBF, ExpSineSquared, DotProduct, RationalQuadratic,
    WhiteKernel, ConstantKernel, Kernel
)
...
def generate_multivariate_time_series(dimensions=3, max_kernels=5, efficient=True):
    while True:
        X = np.linspace(0, 1, LENGTH)

        # Randomly select up to max_kernels kernels from the KERNEL_BANK
        ...
        # Sample a multivariate time series from the GP prior
        try:
            ts = []
            for _ in range(dimensions):
                if efficient:
                    ts.append(sample_from_gp_prior_efficient(kernel=kernel, X=X).squeeze())
                else:
                    ts.append(sample_from_gp_prior(kernel=kernel, X=X).squeeze())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--efficient", action="store_true", default=False, help="Use efficient sampling method")
    args = parser.parse_args()
    path = Path(__file__).parent / "kernelsynth-data.arrow"

    generated_dataset = Parallel(n_jobs=-1)(
        delayed(generate_multivariate_time_series)(dimensions=args.dimensions, max_kernels=args.max_kernels, efficient=args.efficient)
